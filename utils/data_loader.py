...
class Unsupervised_Train_ChangeDataset(data.Dataset):
    def __init__(self, root, unsupervised_dataset_txt,trainsize, mosaic_ratio=0.2):
        self.trainsize = trainsize

        file_list = os.path.join(root, 'list', unsupervised_dataset_txt)
        img_name_list = np.loadtxt(file_list, dtype=np.str_)
        ...
        self.images_A = [self.image_root_A + f for f in self.files if f.endswith('.jpg') or f.endswith('.png')]
        self.images_B = [self.image_root_B + f for f in self.files if f.endswith('.jpg') or f.endswith('.png')]
        self.gts = [self.gt_root + f for f in self.files if f.endswith('.jpg')
                    or f.endswith('.png')]
        ...
        self.filter_files()
        # transforms
        self.img_resize=transforms.Resize((self.trainsize, self.trainsize))
    ...
    def __getitem__(self, index):
        # enhancing pseudo-label quality by SAM
        if random.random()>0:
            image_A, image_B, gt = self.load_img_and_mask(index)
            image_B_path=self.images_B[index]
            image_A_path = self.images_A[index]
            image_WA, image_WB, image_SA, image_SB, gt = self._strong_augmentation(image_A, image_B, gt)

            image_WA = self.img_transform(image_WA)
            image_SA = self.img_transform(image_SA)
            image_SB = self.img_transform(image_SB)
            image_WB = self.img_transform(image_WB)
            gt = self.gt_transform(gt)
            return image_WA, image_WB, image_SA, image_SB, gt,image_B_path,image_A_path
        ...
        if p_ratio > self.mosaic_ratio:
            ...
            image_A_=image_A.copy()
            image_B_ = image_B.copy()
            gt_=gt.copy()

            image_A, image_B, gt = randomCrop(image_A, image_B, gt)
            for _ in range(10):
                labels, cnt = np.unique(gt, return_counts=True)
                #cnt = cnt[labels != 255]
                if len(cnt) > 1 and np.max(cnt) / np.sum(
                        cnt) < 0.75:
                    break
                image_A, image_B, gt = randomCrop(image_A_, image_B_, gt_)
        ...
        image_A=self.img_resize(image_A)
        image_B = self.img_resize(image_B)
        gt = self.img_resize(gt)
        ...
        image_WB = self.img_transform(image_WB)
        gt = self.gt_transform(gt)
        return image_WA, image_WB, image_SA,image_SB,gt,"no_sam","no_sam"
    ...
    def _strong_augmentation(self, image_A, image_B, label):
        ...
        image_SA = self.strong(image_A)
        image_SB = self.strong(image_B)

        return image_A,image_B,image_SA,image_SB,label
...
def get_unsupervised_train_loader(root, unsupervised_dataset_txt,batchsize, trainsize, num_workers=1, shuffle=True, pin_memory=True):

    dataset =Unsupervised_Train_ChangeDataset(root = root, unsupervised_dataset_txt=unsupervised_dataset_txt,trainsize= trainsize)
...
class Supervised_Train_ChangeDataset(data.Dataset):
    def __init__(self, root, supervised_dataset_txt,trainsize, mosaic_ratio=0.2):
        self.trainsize = trainsize

        file_list = os.path.join(root, 'list', supervised_dataset_txt)
        img_name_list = np.loadtxt(file_list, dtype=np.str_)
        self.files = img_name_list
...
def get_loader(root, supervised_dataset_txt,batchsize, trainsize, num_workers=1, shuffle=True, pin_memory=True):

    dataset =Supervised_Train_ChangeDataset(root = root, supervised_dataset_txt=supervised_dataset_txt,trainsize= trainsize)
    data_loader = data.DataLoader(dataset=dataset,
                                  batch_size=batchsize,
                                  shuffle=shuffle,
                                  num_workers=num_workers,
